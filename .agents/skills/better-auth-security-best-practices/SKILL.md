...
export const auth = betterAuth({
    databaseHooks: {
        session: {
            create: {
                after: async ({ data, ctx }) =>
                    void (await auditLog("session.created", {
                        userId: data.userId,
            ...
            delete: {
                before: async ({ data }) => void (await auditLog("session.revoked", { sessionId: data.id })),
            },
        },
        user: {
            update: {
        ...
        account: {
            create: {
                after: async ({ data }) =>
                    void (await auditLog("account.linked", {
                        userId: data.userId,
...
export const auth = betterAuth({
    ...
    databaseHooks: {
        session: {
            create: {
                after: async ({ data, ctx }) => void console.log(`New session for user ${data.userId}`),
            },
        },
        user: {
            update: {
                after: async ({ data, oldData }) => {
                    if (oldData?.email !== data.email) console.log(`Email changed for user ${data.id}`)
                },
            },
        },
    },
