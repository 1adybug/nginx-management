...
function handleSearch(query: string) {
    ...
    requestIdleCallback(() => void analytics.track("search", { query }))

    requestIdleCallback(() => void saveToRecentSearches(query))

    requestIdleCallback(() => void prefetchTopResults(results.slice(0, 3)))
}
```
