...
- 完成较大范围的修改后，必须先确认项目已有的验证脚本，并根据改动范围执行适用的类型检查、静态检查和相关测试，例如 `typecheck`、`lint` 或 `test`。
- 除非用户明确要求，不得使用构建或打包命令进行验证，也不得以此替代上述检查。
- 发现问题时，如项目配置了 `template` 远程仓库，必须先判断问题源于上游模板还是当前项目的特有实现。
- 若问题源于上游模板，应暂停修复并向用户说明；若问题仅存在于当前项目，则按正常流程继续处理，无需特别说明。
