...
export const GeshuAgentBindingPage: FC = () => {
    ...
    useEffect(() => {
        ...
        if (linkResult === "error")
            toast.error(error ? getOAuthBindingErrorMessage(error, description) : "geshu-agent 账户绑定没有成功，请重新尝试。", { id: toastId })
    ...
    async function linkAccount() {
        ...
        if (!loginStatus?.ready) {
            toast.error("暂时无法绑定 geshu-agent，请联系管理员处理。")
            return
        }
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>绑定 geshu-agent</CardTitle>
                <CardDescription>
                    {user ? "当前本平台账户已登录，请继续完成 geshu-agent 登录和授权。" : "该 geshu-agent 尚未关联本平台账户。请先登录本平台，再继续授权绑定。"}
                </CardDescription>
            </CardHeader>
            <CardContent>
                {user ? (
